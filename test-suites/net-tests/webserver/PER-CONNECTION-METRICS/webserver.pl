...
open(GRAPHITE, "| ../../../../common/nc -w 120 $carbon_server $carbon_port") || die "failed to send: $!\n";
...
foreach my $connections (@CONNECTIONS){
  ...
  while ($loops-- > 0 ) {
   open (INTERFACE, " ../../../../common/wrk --latency -t $wthreads -c $connections -d 30s http://$peer:$webserver_port/$filename |")|| die print "failed to get data: $!\n";
   while (<INTERFACE>) {
	...
	push @data, "$server-netbench.$host.benchmark.webserver.CONNECTIONS.$stats[0] $stats[1]  $now \n";
	}
       if ($stats[1] =~ /ms/){
   ...
   if (/^Requests/){
	...
	push @data, "$server-netbench.$host.benchmark.webserver.CONNECTIONS.RPS $stats[1]  $now \n";
   }
  }
  close(INTERFACE);
