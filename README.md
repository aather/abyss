...
It is prefered to clone it on multiple servers of any type: VirtualBox, Cloud Instance, Bare-matel ..
...
```
 carbon_server   	Agents ships metrics to carbon or graphite server
 carbon_port     	Agent sends request to this Port number where carbon or graphite server is listening. Default: 7405
...
- IO Benchmark Metrics:
  Benchmark agent sets up the requested filesystem on device(s) and mount it. Run series of tests (latency, direct, sequential, random) 
  using fio and captures: iops, throughput and latencies from the results. 
